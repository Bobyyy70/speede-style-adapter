-- Adapter execute_sql_admin pour ne plus exiger auth.uid()
-- Garde-fous conservés : whitelist DDL, blocage schémas réservés, interdiction ALTER DATABASE
...
DECLARE
  ...
  v_schema_name text;
BEGIN
  -- Plus de vérification auth.uid() - l'authentification se fait via x-mcp-key au niveau de l'endpoint
