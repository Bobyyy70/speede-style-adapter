...
// Icons imports
import { LayoutDashboard, Settings, LogOut, Search, Menu, X, Bell, ArrowRightLeft, ClipboardList, RefreshCw, MapPin, Undo2, TruckIcon, Plug, FolderTree, Boxes, Warehouse, PackageOpen, PackagePlus, UserCog, Receipt, ArrowDownUp, ShipWheel, Cable, Building2, Eye, ChevronDown, PlusCircle, Headphones, Wrench } from "lucide-react";
      ...
      <aside className={cn("fixed inset-y-0 left-0 z-50 bg-card border-r border-border transition-all duration-300", sidebarOpen ? "w-64" : "w-24")}>
        ...
        <nav className="flex-1 space-y-4 p-6 overflow-y-auto">
          ...
          return <div key={item.name}>
                {item.children ? <div className="space-y-1">
                    <button onClick={() => {
                if (sidebarOpen) {
              ...
              }} className={cn("w-full flex items-center rounded-lg text-sm font-medium transition-all duration-200 h-11", sidebarOpen ? "px-4 gap-3 justify-start border-l-4" : "px-2 gap-0 justify-center border-l-[5px]", isActive ? "bg-primary/10 text-primary border-primary shadow-sm" : "text-muted-foreground hover:bg-secondary hover:text-foreground hover:border-primary/50 border-transparent")}>
                      <item.icon className="w-6 h-6 flex-shrink-0" />
                      {sidebarOpen && <span className="flex-1 text-left">{item.name}</span>}
                    </button>
                    {sidebarOpen && isExpanded && <div className="ml-8 space-y-2 mt-2">
                        {item.children.map(child => {
                  const isChildActive = location.pathname === child.href;
                  return <Link key={child.name} to={isViewingAsClient() ? `${child.href}${location.search}` : child.href} className={cn("flex items-center gap-3 rounded-lg px-4 py-2.5 text-sm font-medium transition-all duration-200", isChildActive ? "bg-primary text-primary-foreground shadow-lg scale-105" : "text-muted-foreground hover:bg-secondary hover:text-foreground hover:scale-102")}>
                              <child.icon className="w-5 h-5 flex-shrink-0" />
                              <span>{child.name}</span>
                            </Link>;
                })}
                      </div>}
                  </div> : <Link to={isViewingAsClient() ? `${item.href}${location.search}` : item.href!} onClick={e => {
      ...
      <div className={cn("transition-all duration-300", sidebarOpen ? "ml-64" : "ml-24")}>
        ...
        <header className={`sticky z-40 bg-card border-b border-border ${isActuallyViewingAsClient ? 'top-[48px]' : 'top-0'}`}>
          <div className="flex h-16 items-center justify-between px-6">
            <div className="flex items-center flex-1 gap-4 max-w-md">
              <Popover open={searchOpen} onOpenChange={setSearchOpen}>
                ...
                <PopoverContent className="w-[400px] p-0 z-50 bg-card border shadow-lg" align="start">
                  <Command>
                    <CommandInput placeholder="Rechercher produits, ordres, emplacements..." value={searchQuery} onValueChange={setSearchQuery} />
                    <CommandList>
                      <CommandEmpty>
                        {searchLoading ? "Recherche en cours..." : "Aucun résultat trouvé."}
                      </CommandEmpty>
                      {searchResults.length > 0 && <CommandGroup heading="Résultats">
                          {searchResults.map(result => <CommandItem key={result.id} onSelect={() => {
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-3 ml-4 pl-4 border-l border-border">
                ...
                <Button variant="ghost" size="icon" onClick={signOut} className="h-8 w-8" title="Déconnexion">
          ...
          <div className="flex h-12 items-center border-t px-6 bg-muted/50">
            <Breadcrumb>
              <BreadcrumbList>
                {breadcrumbs.map((crumb, index) => <div key={crumb.href} className="flex items-center">
                    {index > 0 && <BreadcrumbSeparator />}
                    <BreadcrumbItem>
                      {index === breadcrumbs.length - 1 ? <BreadcrumbPage>{crumb.label}</BreadcrumbPage> : <BreadcrumbLink asChild>
