...
const AVAILABLE_RELATIONS = [
  {
    ...
    label: "Commande",
    fields: [
      { value: "statut_wms", label: "Statut", type: "select" as const, options: [
        { value: "en_attente_reappro", label: "En attente réappro" },
        { value: "stock_reserve", label: "Stock réservé" },
        { value: "en_picking", label: "En picking" },
        { value: "pret_expedition", label: "Prêt expédition" }
      ]},
      { value: "priorite_expedition", label: "Priorité", type: "select" as const, options: [
...
export default function ReglesFiltrage() {
  ...
  const [formData, setFormData] = useState({
    nom_regle: "",
    description: "",
    type_session: "standard",
    priorite: 100
  });
  const [conditions, setConditions] = useState<Condition[]>([]);

  const { data: regles, isLoading } = useQuery({
    queryKey: ["regles-session-preparation"],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("regle_session_preparation")
        .select("*")
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["regles-session-preparation"] });
  ...
  const resetForm = () => {
    setFormData({ nom_regle: "", description: "", type_session: "standard", priorite: 100 });
    setConditions([]);
    setEditingRule(null);
  ...
  const handleEdit = (regle: any) => {
    ...
    setFormData({
      ...
      type_session: regle.type_session,
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Règles de Filtrage des Sessions</h1>
            <p className="text-muted-foreground mt-2">
              Définissez comment créer automatiquement des sessions de préparation selon les caractéristiques des commandes
            </p>
          </div>
          ...
          <CardContent>
            {isLoading ? (
              ...
              <div className="text-center py-8 text-muted-foreground">
                ...
                <p>Aucune règle définie. Créez votre première règle pour automatiser la création de sessions.</p>
              </div>
            )}
          </CardContent>
        </Card>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editingRule ? "Modifier la règle" : "Nouvelle règle de filtrage"}</DialogTitle>
            <DialogDescription>
              Définissez les conditions pour créer automatiquement une session de préparation
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="Ex: Commandes urgentes Client X"
                />
              </div>

              <div className="space-y-2">
                <Label htmlFor="type_session">Type de session</Label>
                <Select
                  value={formData.type_session}
                  onValueChange={(value) => setFormData({ ...formData, type_session: value })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    ...
                    <SelectItem value="client_dedie">Client dédié</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </div>
