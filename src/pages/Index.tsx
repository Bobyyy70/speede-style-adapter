...
const Index = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      const { data: commandes } = await supabase
        .from("commande")
        .select("statut_wms, date_creation, tracking_number");
      ...
      const tauxLivraison = commandesAvecTracking.length > 0 
        ? Math.round((expediees / commandesAvecTracking.length) * 100) 
        : 0;

      // Erreurs de préparation - basé sur retours avec raison erreur
      const { data: retoursErreur } = await supabase
        .from("retour_produit")
        .select("raison_retour")
        .ilike("raison_retour", "%erreur%");
      
      const tauxErreurs = commandes && commandes.length > 0
        ? Math.round((retoursErreur?.length || 0) / commandes.length * 100)
        : 0;
  ...
  useEffect(() => {
    const fetchChartData = async () => {
      ...
      const { data: commandes } = await supabase
        .from("commande")
        .select("date_creation, statut_wms")
        .gte("date_creation", startDate.toISOString());

      const groupedData: Record<string, { date: string; commandes: number; expeditions: number }> = {};
      
      for (let i = 0; i < days; i++) {
        const date = subDays(new Date(), days - i - 1);
        const dateStr = format(date, "dd/MM");
        groupedData[dateStr] = { date: dateStr, commandes: 0, expeditions: 0 };
      }
  ...
  return (
    <DashboardLayout>
      <div className="space-y-4">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          <StatCard
            title="Taux Livraison à l'Heure"
            value={`${stats.tauxLivraisonHeure}%`}
            icon={TrendingUp}
            variant="success"
          />
          <StatCard
            title="Taux Erreurs Préparation"
            value={`${stats.tauxErreurs}%`}
            icon={XCircle}
            variant="default"
          />
          <StatCard
          ...
          />
          <StatCard
            title="Délai Moyen Prépa (h)"
            value={stats.delaiMoyenPreparation}
            icon={Clock}
            variant="primary"
          />
        </div>
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          ...
          <Card>
            ...
            <CardContent>
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={[
                  { name: "En préparation", value: stats.commandesEnPreparation },
                  { name: "Expédiées", value: stats.commandesExpediees },
                  { name: "Retours", value: stats.retoursEnCours },
                ]}>
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis dataKey="name" />
                  <YAxis />
